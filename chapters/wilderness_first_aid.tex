...
\section{Improvised Stretchers}
When evacuation is necessary and commercial stretchers are unavailable, improvise one from gear on hand. The goal is to spread the load, protect the spine, and allow safe transport over rough terrain.

\subsection{Shirt Stretcher}
The simplest improvised stretcher requires two people and a sturdy shirt or jacket:
\begin{enumerate}
  \item Lay the shirt face-down on the ground. Have the injured person roll onto the shirt on their back.
  \item Two rescuers stand on opposite sides. Each rescuer grasps the shirt at the shoulder and hip areas, turning the hem inward to form handles.
  \item Lift smoothly, keeping the person level. The shoulder handles support the upper body; the hip handles support the pelvis and legs.
  \item Walk in step, with the head-end rescuer slightly ahead to monitor the person's airway and comfort.
\end{enumerate}
\textbf{WARNING}: Do not use a shirt stretcher for suspected spinal injuries -- use a rigid improvisation (see below) or wait for professional help.

\subsection{Pole and Shelter Stretcher}
For longer carries or rough terrain, a rigid stretcher is safer:
\begin{enumerate}
  \item Use two sturdy poles (trekking poles, branches 2--3 meters long) parallel to each other.
  \item Lay a tarp, space blanket, or folded jacket across the poles.
  \item Secure the material to the poles with cord, belts, or clothing strips, tying square knots at each corner and along the edges.
  \item Roll the person onto the stretcher using a log-roll with at least three helpers if spinal injury is suspected: one person maintains manual in-line stabilization of the head and neck, while the others roll the person as a single unit onto the stretcher.
\end{enumerate}
\textbf{Tip}: Pad the stretcher surface with clothing or a sleeping pad for comfort and to prevent pressure sores on long carries.

\subsection{Litter from Gear}
A backpack can serve as a rigid base:
\begin{enumerate}
  \item Place the backpack on its side between two poles threaded through the compression straps or handle loops.
  \item Lay a jacket or tarp over the pack and poles, securing it tightly.
  \item The backpack provides a firm platform; the poles serve as handles for the rescuers.
\end{enumerate}
Always test an improvised stretcher before loading a patient: lift each corner to check for stress points and re-tie as needed.
